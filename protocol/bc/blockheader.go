...
func (bh *BlockHeader) writeForHash(w io.Writer) {
	...
	mustWriteForHash(w, bh.StateRoot)
...
// its body.
func NewBlockHeader(version, height uint64, previousBlockID *Hash, timestamp uint64, transactionsRoot, transactionStatusHash *Hash, stateRoot *Hash, nonce, bits uint64) *BlockHeader {
	return &BlockHeader{
		Version:               version,
		Height:                height,
